...
flavor = {0:'nu_e',1:'nu_e_bar',2:'nu_mu',3:'nu_mu_bar',4:'nu_tau',5:'nu_tau_bar'}
##############################Multiprocessing##################################
def process(func, args, ncpu):
	...
	q = multip.Queue()
	p = [multip.Process(target=wrapper, args=(func, a, i, q)) 
		for i, a in enumerate(np.array_split(args, ncpu)) if a.size > 0]
...
def SunZenith(MJD,lat_det):
	...
	t       = Time(MJD, format='mjd', scale='utc',location=(0.*u.deg, lat_det*u.deg))
	h       = np.deg2rad(15*((float(t.iso[11:13])+float(t.iso[14:16])/60.+float(t.iso[17:19])/3600.)-12.))
	cos     = np.sin(np.deg2rad(lat_det))*np.sin(delta)+np.cos(np.deg2rad(lat_det))*np.cos(delta)*np.cos(h)  
...
def Distance(zenith):
	...
	x          = Symbol('x')
	solution   = solve((x*x+r_earth*r_earth-AU*AU)/(2*x*r_earth)-sym.cos(sym.pi-zenith),x)
...
def Pack(ch,DMm,mass_v,folder):
	...
	"""
	def key(s):
	    return float(s[:-s[::-1].find('_') - 1][-s[:-s[::-1].find('_') - 1][::-1].find('_'):])
...
def IniFluxFunction(ch,DMm,process='ann',wimp_loc='Sun',path=None,secluded=False,mass_v=1000.):
	...
	if secluded == False:
		...
		if path == None:	
			...
			x    = data['x'][:]/factor
			for k in range(6):
				f.append(interp2d(x,mass[mass>=p_mass[ch]],flux_data[:,k,:],bounds_error=False,kind='linear'))
			data.close()
		elif os.path.isfile(path):
			data  = np.genfromtxt(path)
			print ('Initial Flux Loading: '+path)            
			for k in range(6):
				f.append(interp1d(data[:,0]/DMm,data[:,k+1],fill_value='extrapolate'))                    
		elif os.path.isdir(path):
			for k in range(6):
				if len(glob.glob(path+'/{}_{}_{}_{}_*-{}.dat'.format(ch,DMm,wimp_loc,process,k)))==0:
						sys.exit('Initial flux not found')
				else:                        
					for files in glob.glob(path+'/{}_{}_{}_{}_*-{}.dat'.format(ch,DMm,wimp_loc,process,k)):
						data  = np.genfromtxt(files)
						print ('Initial Flux Loading: '+files)                     
					f.append(interp1d(data[:,0]/DMm,data[:,1],fill_value='extrapolate'))
		return f 
		...
		if os.path.isfile(path):
			...
			if os.path.isfile(path+'/{}_{:.1f}_{:.1f}.npy'.format(ch,DMm,mass_v)):
				...
				print ('Initial Flux Loading: '+path+'/{}_{:.1f}_{:.1f}.npy'.format(ch,DMm,mass_v))               
			else:
		...
		f_vacuum = []
		for k in range(6):
			f.append(interp2d(data['x'][:,0],data['rho'][0][1:],2.*np.transpose(data[flavor[k]][:,1:]),bounds_error=False))
			f_vacuum.append(interp1d(data['x'][:,0],2.*data[flavor[k]][:,0],fill_value = 'extrapolate'))
		return f, f_vacuum 

def IniFlux(Enu,ch,DMm,process='ann',wimp_loc='Sun',secluded=False,r = None,mass_v=1000.,pathFlux=None,pathSunModel=None,pathEarthModel=None):
	...
	nuflavor = {'nu_e':1,'nu_e_bar':2,'nu_mu':3,'nu_mu_bar':4,'nu_tau':5,'nu_tau_bar':6} 
	nuID     = {'nu_e':12,'nu_e_bar':-12,'nu_mu':14,'nu_mu_bar':-14,'nu_tau':16,'nu_tau_bar':-16} 
	if not secluded:
		flux = np.zeros((len(Enu)),dtype=[('nu_e','float'),('nu_mu','float'),
	           ('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'), ('nu_tau_bar','float')])
		f =   IniFluxFunction(ch,DMm,process=process,wimp_loc=wimp_loc,path=pathFlux,secluded=secluded)
		if pathFlux == None:
		...
		f, f_vacuum =   IniFluxFunction(ch,DMm,process='decay',wimp_loc=wimp_loc,path=pathFlux,secluded=secluded,mass_v=mass_v)                    
		if type(r) in [float,int]:
			r = [r]
		flux = np.zeros((len(r),len(Enu)),dtype=[('nu_e','float'),('nu_mu','float'),
	           ('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'), ('nu_tau_bar','float')])
		for k in flux.dtype.names:
			if   wimp_loc == 'Sun':           
			...
			if len(rho[rho>0.]) > 1:
				flux_in  = np.flip(f[nuflavor[k]-1](Enu/DMm,rho[rho>0.]),axis=0) 
			elif len(rho[rho>0.]) == 1:
				flux_in  = f[nuflavor[k]-1](Enu/DMm,rho[rho>0.]) 
			else:
				flux_in  = np.array([])
			if len(rho[rho==0.])>0.:
				flux_out = np.array([f_vacuum[nuflavor[k]-1](Enu/DMm)]*len(rho[rho==0.]))
			else:
				flux_out = np.array([])
...
def average(nuSQ,Eout,flavor):
	...
	for i,E in enumerate(Eout):
		if i == 0.:	
			E_list = np.linspace(E,E+Ediff[0]/2.,10)
		elif i == bins-1:
			E_list = np.linspace(E-Ediff[-1]/2.,E,10)
		else:
			E_list = np.linspace(E-Ediff[i]/2.,E+Ediff[i]/2.,10)
		flux.append(np.mean(np.array([nuSQ.EvalFlavor(flavor[0],e,flavor[1]) for e in E_list])))
...
def propagate(iniflux,Ein,Eout,location_ini,location_end,theta_12=33.82,theta_23=48.6,theta_13=8.60,delta_m_12=7.39e-5,delta_m_13=2.528e-3,delta=0.,interactions=True,xsec = None,secluded=False,r=0.0,mass_v=0.,zenith =0.,avg=True,pathSunModel=None,pathEarthModel=None):
	...
	flux_output = np.zeros(len(Eout),dtype = [('Energy','float'),('nu_e','float'),('nu_mu','float'),('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'),('nu_tau_bar','float'),('zenith','float')]) 
	# Standard case 	
	if not secluded:
		# from Sun 
		if location_ini == 'Sun':
			...
			nuSQ.Set_Track(nsq.Sun.Track(0.,pc.SUNRADIUS*pc.km))
		...
		# from Earth 
			...
			if location_end == 'detector':
				...
				#nuSQ.Set_Body(nsq.Earth())
				nuSQ.Set_Track(nsq.Earth.Track(pc.EARTHRADIUS*pc.km,2*pc.EARTHRADIUS*pc.km,2*pc.EARTHRADIUS*pc.km))
				nuSQ.Set_initial_state(iniflux,nsq.Basis.flavor)
				nuSQ.EvolveState()
		...
		# from Halo 
			...
			composition_nu    = np.array([u_to_fr([iniflux[j,0,0],iniflux[j,0,1],iniflux[j,0,2]],osc_matrix) for j in range(len(Ein))])
			composition_nubar = np.array([u_to_fr([iniflux[j,1,0],iniflux[j,1,1],iniflux[j,1,2]],osc_matrix) for j in range(len(Ein))])

			
			# flux at Earth surface before entering matter
			if location_end == 'Earth' or zenith <= np.pi/2.:
				for i in range(3):
	        ...
	        # flux at detector
				...
				if   pathEarthModel == None:
					...
					nuSQ.Set_Body(nsq.Earth(pathEarthModel))                 
				#nuSQ.Set_Body(nsq.Earth())
				nuSQ.Set_Track(nsq.Earth.Track(2*abs(np.cos(zenith))*pc.EARTHRADIUS*pc.km))
	...
	#Secluded case 	
		...
		if location_ini == 'Sun':
			...
			if r < pc.SUNRADIUS:
				...
				if b_impact !=0:
					...
					if   pathSunModel == None:
						nuSQ.Set_Body(nsq.SunASnu(dirpath+'/models/struct_b16_agss09.dat'))
					else:  
						nuSQ.Set_Body(nsq.SunASnu(pathSunModel))                    
					nuSQ.Set_Track(nsq.SunASnu.Track(l*pc.km,xini*pc.km,b_impact*pc.km))
			...
			# flux at detector 
				...
				nuSQ.Set_Body(nsq.Vacuum())
				if r < pc.SUNRADIUS:
					nuSQ.Set_Track(nsq.Vacuum.Track(xini_Sun(b_impact,r,zenith)[1]*pc.km))
				elif r >= pc.SUNRADIUS and r < pc.AU/pc.km-pc.EARTHRADIUS:
					nuSQ.Set_Track(nsq.Vacuum.Track(xini_Sun(b_impact,r,zenith)[0][1]*pc.km))
					nuSQ.Set_initial_state(iniflux,nsq.Basis.flavor)
				nuSQ.EvolveState()
				if   pathEarthModel == None:
					nuSQ.Set_Body(nsq.EarthAtm())
		...
		# from the Earth		
				...
				if   pathEarthModel == None:
					...
					nuSQ.Set_Body(nsq.Earth(pathEarthModel)) 
				nuSQ.Set_Track(nsq.Earth.Track(xini*pc.km,2*pc.EARTHRADIUS*pc.km,2*pc.EARTHRADIUS*pc.km))
				nuSQ.Set_initial_state(iniflux,nsq.Basis.flavor)
				nuSQ.EvolveState()
...
def wrap_propagate(x,params):
	...
	return propagate(iniflux,params['Ein'],params['Eout'],params['location_ini'],params['location_end'],r=r,theta_12=params['theta_12'],theta_13=params['theta_13'],theta_23=params['theta_23'],delta_m_12=params['delta_m_12'],delta_m_13=params['delta_m_13'],delta=params['delta'],interactions=params['interactions'],xsec=params['xsec'],secluded=True,mass_v=params['mass_v'],zenith=params['zenith'],avg=params['avg'],pathSunModel=params['pathSunModel'],pathEarthModel=params['pathEarthModel'])
...
def Probability_point(x,decayLength_v):
	...
	"""	
	prob          = 1./decayLength_v*np.exp(-x/decayLength_v)
	return prob
...
##############################Generate fluxes##################################
class NuFlux:
	def __init__(self,ch,DMm, nodes,Emin = 0.,Emax = None, bins = None, logscale=False, process='ann',theta_12=33.82,theta_23=48.6,theta_13=8.60,delta_m_12=7.39e-5,delta_m_13=2.528e-3,delta=0.,interactions=True,xsec=None,pathFlux=None, pathSunModel=None,pathEarthModel=None):
		self.flavor_list    = {0:'nu_e',1:'nu_e_bar',2:'nu_mu',3:'nu_mu_bar',4:'nu_tau',5:'nu_tau_bar'}	
		self.DMm            = DMm
		self.ch             = ch
		self.nodes          = nodes 
	...
	def iniFlux(self,wimp_loc):
		...
		wimp_loc   :	string_
						location of neutrino production
		"""
		return IniFlux(self.params['Ein'],self.ch,self.DMm,process=self.process,wimp_loc=wimp_loc,pathFlux=self.pathFlux) 
	...
	def iniFlux_secluded(self,wimp_loc,mass_v,r):
		...
		mass_v      	:	float
					 	    mediator_mass in GeV
		"""
		return IniFlux(self.params['Ein'],self.ch,self.DMm,process='decay',wimp_loc=wimp_loc,pathFlux=self.pathFlux,pathSunModel=self.pathSunModel,pathEarthModel=self.pathEarthModel,secluded=True,r=r,mass_v=mass_v)
	...
	def Sun(self,location_end,time=57754.,zenith=None,latitude=-90.,avg=True):
		...
		if zenith == None:
			...
			zenith = zenith 
		return propagate(iniflux = initial_flux,location_ini='Sun',location_end=location_end,secluded=False,zenith =zenith,avg=avg,**self.params)
	...
	def Earth(self,location_end,avg=True):
		...
		return propagate(iniflux = initial_flux,location_ini='Earth',location_end=location_end,secluded=False,avg=avg,**self.params)
	
	
	def Halo(self,location_end,zenith=0.,avg=True):
		...
		return propagate(iniflux=initial_flux,location_ini='Halo',location_end=location_end,secluded=False,avg=avg,zenith=zenith,**self.params)
	

	def SunSecluded(self,segment,lambda_v,mass_v,location_end,ncpu=1,time=57754.,zenith=None,latitude=-90.,avg=False,p=0.9,approx_1D=True):
		r"""propagate flux from the Sun
		Parameters
		----------
		segment         :  int
		...
		r		     = np.linspace(0.,xmax,segment+1)
		r_center     = (r[1:]+r[:-1])/2.
		...
		fluxes  =  pool.map(partial(wrap_propagate,params=ss_params), [(initial_flux[i], r_center[i]) for i in range(segment)])
		results = np.zeros((self.bins),dtype=[('Energy','float'),('nu_e','float'),('nu_mu','float'),('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'),('nu_tau_bar','float'),('zenith','float')])
		if approx_1D:
			prob    = Probability_length(r[:-1],r[1:],lambda_v)
			
			for j in results.dtype.names:
				if j != 'Energy' or j!= 'zenith':
					results[j] = sum([fluxes[k][j]*prob[k] for k in range(len(r_center))])
	...
	def EarthSecluded(self,segment,lambda_v,mass_v,location_end,zenith=None,p=0.9,avg=False,approx_1D=True,ncpu=1):
		xmax         = -lambda_v*np.log(1-p)
		if approx_1D:
		...
		pool    = multip.Pool(processes=ncpu)
		fluxes  = pool.map(partial(wrap_propagate,params=es_params), [(initial_flux[i], r_center[i]) for i in range(segment)])
		results = np.zeros((self.bins),dtype=[('Energy','float'),('nu_e','float'),('nu_mu','float'),('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'),('nu_tau_bar','float'),('zenith','float')])
		if approx_1D:
			prob    = Probability_length(r[:-1],r[1:],lambda_v)
			for j in results.dtype.names:
